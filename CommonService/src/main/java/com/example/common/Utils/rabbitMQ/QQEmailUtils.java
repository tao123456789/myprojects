...
public class QQEmailUtils {
    ...
    public void SendToByQQEmail(EmailPO emailPO) throws Exception {
        ...
        try {
            ...
            //
            message.setContent(emailPO.getContent(),"text/html;charset=UTF-8");
            //设置发件人邮箱地址
            message.setFrom(new InternetAddress(QQAccount));
            //设置收件人地址
            message.setRecipients(MimeMessage.RecipientType.TO, new InternetAddress[]{new InternetAddress(emailPO.getToEmail())});
            //设置邮件标题
            message.setSubject(emailPO.getSubject());
            //设置邮件内容
            message.setText(emailPO.getContent());
            ...
            System.out.println("发给 "+emailPO.getToEmail()+" 的邮件发送失败： "+e);
